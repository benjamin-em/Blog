...
当我们从预训练的网络创建模型时，fastai会自动为我们冻结所有预训练的图层。 当我们调用fine_tune方法时，fastai做两件事：
 - 冻结其他层,对加入的层进行一个周期(epoch)的训练.
 - 解冻其他层,对全部层,进行进行训练  
默认方法虽然很合理,不过对我们特定数据集来说,稍微做一点调整可能会得到更好的结果. ```fine_tune```方法有很多参数,可以用这个语句查一下：
```
learn.fine_tune??
```
试下手动执行这个操作.首先,用```fit_one_cycle```对随机添加的层做三个周期的训练. 不用```fine_tune```的话, 推荐用```fit_one_cycle```的方式.后面会看到这是为什么,先看看```fit_one_cycle```, 以一个比较低的学习率开始,在训练的前一部分逐步增加,然后在剩下的部分逐渐减小.
```
learn = cnn_learner(dls, resnet34, metrics=error_rate)
...
learn.unfreeze()
```
然后再次运行```lr_find```,因为有更多的层需要训练,权重已经训练过3个周期,这意味着之前的学习率已经不再合适.
```
learn.lr_find()
```
...
即使我们解冻后，我们仍然非常关心这些预训练权重的质量. 即使我们已经将这些随机添加的参数调整了几个周期，我们也不希望这些预训练参数的最佳学习率达到随机添加的参数的最佳学习率那么高. 预训练的权重是在数百万个图像上,数百个周期训练后得到的。
