...
根据自述文件, 主表位于文件u.data中. 它是制表分隔的, 列分别是用户, 电影, 评级和时间戳. 由于这些名称未编码, 所以用Pandas读取这个文件时需要指出它们. 下面是打开表并查看的一种方式:
...
虽然它包含所有我们需要的信息, 但是对人为读取数据来说,不是非常有帮助. 下表展示了显示与人类友好的表中交叉表现相同的数据.
