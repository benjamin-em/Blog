...
如果知道每个用户喜欢的电影在多大程度上可能落入每个重要类别(如类型, 年龄, 首选导线和演员等), 并且知道关于每部电影的相同信息, 那么有个方法可以很简单的填入此表格: 将此信息乘以每部电影并使用组合. 例如, 假设这些因素落在-1到+1之间的系数, 正数表示具有较强的匹配, 负数表示较弱的匹配, 而这些类别分别是科幻片, 动作片和怀旧电影, 我们可以将 _The Last Skywalker_ 表示为:

```python
last_skywalker = np.array([0.98, 0.9, -0.9])
```

这里例子里面, 我们给_科幻程度_打分为0.98, _动作程度_打分为0.9, 但是电影_非常不怀旧_,打分-0.9. 我们可以这样表示一个用户喜欢现代科幻动作片:
